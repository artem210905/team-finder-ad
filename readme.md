...
**Для корректной проверки проекта, пожалуйста, убедитесь, что:**
1. В вашем файле `.env` установлен параметр `TASK_VERSION=1`.
2. База данных PostgreSQL запущена через `docker compose up -d`.
3. Применены все миграции командой `python manage.py migrate`.
