...
def project_list(request):
    ...
    # Отдаем html шаблон и передаем в него список проектов
...
def complete_project(request, pk):
    project = get_object_or_404(Project, pk=pk)
    # Проверяем, что нажал автор и проект еще открыт
...
def toggle_favorite(request, pk):
    ...
    if favorited := request.user.favorites.filter(id=project.id).exists():
